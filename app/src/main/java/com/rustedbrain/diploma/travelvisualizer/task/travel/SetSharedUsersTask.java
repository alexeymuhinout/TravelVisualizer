...
public class SetSharedUsersTask extends AsyncTask<Void, Void, ResponseEntity<TravelDTO>> {
    ...
    public SetSharedUsersTask(AuthUserDTO userDTO, String username, String travelName, Collection<String> usernames, Listener listener) {
    ...
    protected ResponseEntity<TravelDTO> doInBackground(Void... params) {
        try {
            ...
            return restTemplate.postForEntity(new URI(TravelAppUtils.getAbsoluteUrl(TravelAppUtils.TRAVEL_SHARED_SET_URL)),
    ...
    protected void onPostExecute(final ResponseEntity<TravelDTO> responseEntity) {
        for (Listener listener : taskListeners) {
            listener.setSetSharedUsersTask(null);
            listener.showSetSharedUsersTaskProgress(false);
        }

        if (responseEntity == null) {
            for (Listener listener : taskListeners) {
                listener.showSetSharedUsersTaskError();
            }
        } else if (HttpStatus.OK.equals(responseEntity.getStatusCode())) {
            for (Listener listener : taskListeners) {
                listener.showTravel(responseEntity.getBody());
            }
        } else {
            for (Listener listener : taskListeners) {
                listener.showSetSharedUsersTaskError();
    ...
    public interface Listener {
        ...
        void setSetSharedUsersTask(SetSharedUsersTask setSharedUsersTask);

        void showSetSharedUsersTaskProgress(boolean show);

        void showSetSharedUsersTaskError();

        void showTravel(TravelDTO travelDTO);
    }
}